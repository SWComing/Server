...
function Slash_cm( npcName )
	...
	if myUnit == nil then
		...
		g_pGameEdit:AddString( CHAT_TYPE["CT_KILL"], "Meu papel não pode ser encontrado." )
		return nil
...
function Slash_stage( iStageIndex )
	...
	if g_pX2Game:GoStage( iStageIndex, iSubStageIndex ) then
		...
		g_pGameEdit:AddString( CHAT_TYPE["CT_NORMAL"], "Passe em movimento é bem sucedido." )
	
	else
		g_pGameEdit:AddString( CHAT_TYPE["CT_KILL"], "Passe em movimento falhou." )
